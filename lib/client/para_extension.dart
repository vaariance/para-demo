...
class Parra extends Para {
  ...
  bool _transmissionKeysharesLoaded = false;
  ...
  ParaFuture<SignatureResult> signMessageRaw({
    ...
    final future = (() async {
      // Ensure transmission keyshares are loaded before attempting to sign
      await _ensureTransmissionKeysharesLoaded();
  ...
  Future<void> _ensureTransmissionKeysharesLoaded() async {
    if (!_transmissionKeysharesLoaded) {
      try {
        Logger.warning('Auto-loading transmission keyshares...');
        final sharesLoaded = await loadTransmissionKeyshares();
        if (sharesLoaded == 0) {
          Logger.warning('No transmission keyshares available to load');
        }
        if (sharesLoaded > 0) {
          _transmissionKeysharesLoaded = true;
        }
      } catch (e) {
        // Log the error but don't fail the operation - some operations might still work
        Logger.warning('Failed to auto-load transmission keyshares: $e');
      }
    }
  }
}
